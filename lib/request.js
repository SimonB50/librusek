...
const Request = {
  ...
  async get(url, options = {}) {
    ...
    if (options.pagination) {
      urlObj.searchParams.append("page", options.pagination.page);
      urlObj.searchParams.append("limit", options.pagination.perPage);
    }

    const cacheOptions = options.cache;

    if (cacheOptions && !options.pagination && cacheOptions.enabled) {
    ...
    const response = await fetch(urlObj, {
      ...
      console.error(`Request to ${urlObj.pathname} failed: ${err}`)
    );
    ...
    let data =
      plain_data instanceof Object && !options.customFormat
        ? plain_data[Object.keys(plain_data).find(x => !["resources", "url"].includes(x.toLocaleLowerCase()))]
        : plain_data;
