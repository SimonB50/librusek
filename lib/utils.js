...
const getRealGrade = (grade) => {
  if (grade.includes("+")) return Number(grade.replace("+", "")) + 0.5;
  if (grade.includes("-")) return Number(grade.replace("-", "")) - 0.25;
  return Number(grade);
}
...
const calculateAverage = (grades, categories) => {
  ...
  for (let i = 0; i < grades.length; i++) {
    ...
    if ("CountToTheAverage" in grade && !grade.CountToTheAverage) continue;
    const gradeValue = getRealGrade(grade.Grade);
    if (isNaN(gradeValue) || gradeValue < 1) continue;
    const category = categories.find(
      (category) => category.Id === grade.Category.Id
    );
    if (!category) continue;
    if (category?.IsSemestral || category?.IsSemestralProposition) continue;
    const weight = category.Weight && !isNaN(category.Weight) ? category.Weight : 1;
    top += weight * gradeValue;
    bottom += weight;
  }
...
const uploadContent = async (content, serverUrl = "https://paste.simonb50.xyz") => {
    const response = await fetch(`${serverUrl}/api/v2/paste`, {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
        ...
        return `${serverUrl}/${data?.paste?.id}`;
    } else {
        return null;
    }
}
