 ...
 */
const formatDate = (date, format = 'DD MMMM YYYY, H:mm', locale =  Intl.DateTimeFormat().resolvedOptions().locale) => {
  try {
    dayjs.locale(locale);
...
const decodeAndCleanHtml = (html, addBreaks = true) => {
  ...
  // Replace the message divider with an HTML structure using DaisyUI/Tailwind CSS classes.
  decoded = decoded.replace(
    /---------- Przekazywana wiadomość ---------/g,
    `<hr class="border-t border-gray-300 my-2">
     <div class="pl-5 font-bold">Forwarded Message</div>`
  );

  // Replace newlines with <br /> tags if addBreaks is enabled, otherwise keep as is
  const processedHtml = addBreaks ? decoded.replace(/\n/g, '<br />') : decoded;

  // Sanitize the HTML, allowing only a specific set of safe tags and class names
  return sanitizeHtml(processedHtml, {
    allowedTags: ['p', 'a', 'b', 'i', 'u', 'strong', 'em', 'br', 'div', 'hr'],
    allowedAttributes: {
      div: ['class'],
      hr: ['class']
    },
