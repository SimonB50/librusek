...
const decodeAndCleanHtml = (html, addBreaks = true) => {
  ...
  // Replace newlines with <br /> if addBreaks is enabled, otherwise keep as is
  ...
  // Pass the sanitized content to MessageDivider for further processing
...
const MessageDivider = (content) => {
  ...
  let htmlString = '<div class="space-y-4">';

  // Iterate over the sections and create HTML
  sections.forEach((section, index) => {
    if (index === 0) {
      htmlString += section; // First section, no special styling
    } else {
      htmlString += `
  ...
  // Return the HTML string with sections
  return htmlString;
};


/**
 * Determines the optimal text color (black or white) based on a background color's luminance.
 *
 * @param {string} hexColor - Background color in hex format (e.g., '#RRGGBB' or 'RRGGBB')
 * @returns {string} - Hex code for text color ('#000000' for black or '#FFFFFF' for white)
 * @throws {Error} - If hex color format is invalid
 */
const getTextColor = (hexColor) => {
  const cleanHexColor = sanitizeHexColor(hexColor);
  const rgb = convertHexToRgb(cleanHexColor);
...
/**
 * Removes '#' from hex color and validates its format.
 *
 * @param {string} hexColor - Hex color string to sanitize
 * @returns {string} - Sanitized hex color without '#'
 * @throws {Error} - If hex color format is invalid
 */
const sanitizeHexColor = (hexColor) => {
  const sanitizedColor = hexColor.replace('#', '');

  if (sanitizedColor.length !== 6 || !/^[0-9A-Fa-f]{6}$/.test(sanitizedColor)) {
    throw new Error('Invalid hex color format. Expected 6 characters (e.g., "#RRGGBB" or "RRGGBB")');
  }
 ...
 *
 * @param {string} hex - 6-character hex color string
...
/**
 * Calculates luminance using the YIQ formula.
 *
 * @param {Object} rgb - RGB components { r, g, b }
