...
const calculateAverage = (grades, categories) => {
  let top = 0;
  let bottom = 0;
...
const getNextLesson = (timetableData) => {
  ...
  // only check today lessons
  const dayData = timetableData[currentDate];
  
  if (!dayData) return null;
  
  const lessons = dayData.filter(entry => entry.length > 0)
    .flat()
    .filter(lesson => !lesson.IsCanceled)
    .sort((a, b) => a.HourFrom.localeCompare(b.HourFrom));
  
  // hasnt started yet
  for (const lesson of lessons) {
    if (lesson.HourFrom > currentTime) {
      return {
        ...lesson,
        date: currentDate,
        isToday: true
...
const getRecentGrades = (gradesData, pointsData, daysBack = 3) => {
  if (!gradesData && !pointsData) return [];
  
  const cutoffDate = dayjs().subtract(daysBack, 'day');
  const allGrades = [];
  
  if (gradesData) {
    const recentGrades = gradesData
      .filter(grade => dayjs(grade.AddDate).isAfter(cutoffDate))
      .map(grade => ({ ...grade, type: 'grade' }));
    allGrades.push(...recentGrades);
  }
  
  if (pointsData) {
    const recentPoints = pointsData
      .filter(point => dayjs(point.AddDate).isAfter(cutoffDate))
      .map(point => ({ ...point, type: 'point' }));
    allGrades.push(...recentPoints);
  }
  
  return allGrades.sort((a, b) => dayjs(b.AddDate).valueOf() - dayjs(a.AddDate).valueOf());
};
