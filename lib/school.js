...
const getClassrooms = async (selectors) => {
  if (!selectors || !selectors.length) return false;
  const classroomsData = await Request.get(`${apiUrl}/Classrooms`, {
    cache: { enabled: true, period: 0 },
    selectors,
  });
...
const useClassrooms = (selectors) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getClassrooms(selectors ? selectors.split(",") : "0");
        if (!data) throw new Error("Failed to fetch classrooms data");
        setData(data);
        setError(null);
      } catch (err) {
        setError(err.message);
