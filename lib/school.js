...
const getSubjects = async (id) => {
  if (!id) return false;
  const subjectsRequest = await fetch(`${apiUrl}/Subjects/0,${id}`);
...
const getAnnouncements = async () => {
  const announcementsRequest = await fetch(`${apiUrl}/SchoolNotices`);
  if (!announcementsRequest.ok) return false;
  const announcementsData = await announcementsRequest.json();
